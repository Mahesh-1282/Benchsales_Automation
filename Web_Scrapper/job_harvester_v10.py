...
def fetch_details_parallel(records: list, browser) -> list:
    """
    Fetch job detail pages in parallel using multiple browser pages.
    Only fetches for jobs that passed initial AI scoring.
    """
    ...
    for i in range(0, len(to_fetch), CONFIG["detail_fetch_workers"]):
        ...
        def fetch_one(rec):
            detail = fetch_job_detail(rec["apply_link"], rec["portal"], context)
        ...
        with ThreadPoolExecutor(max_workers=CONFIG["detail_fetch_workers"]) as executor:
            ...
            for future in as_completed(futures):
                try:
                    job_hash, detail = future.result(timeout=40)
    ...
    with sync_playwright() as p:
        ...
        for i, rec in enumerate(fetched_with_desc):
            if rec.get("description_length", 0) > 100:
                ...
                if tech:
                    rec["tech_stack"] = tech
                if exp_yrs and exp_yrs != "Not specified":
                    rec["experience_years"] = exp_yrs
                rec["visa_sponsorship"] = visa
            if (i + 1) % 10 == 0:
